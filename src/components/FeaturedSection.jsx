    ...
    <div className=' overflow-hidden mx-0.5'>
      {/* px-6 md:px-16 lg:px-44 */}
       ...
       <div className='flex flex-wrap max-sm:justify-center gap-5 mt-8 w-full ' >
        {dummyShowsData.slice(0,10).map((show)=>(<MovieCard key={show._id} movie={show}/>))}
       </div>
       <div className='flex justify-center mt-20'>
        <button onClick={()=>{navigate('/movies');window.scrollTo({top:0,behavior:'smooth'})}} className='px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-md font-medium cursor-pointer'>Show More</button>
       </div>
    </div>
  )
}
